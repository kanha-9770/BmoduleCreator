...
interface PermissionMatrixProps {
  ...
  toggleEmployeePermission: (employeeId: string, moduleId: string, permissionType: string) => void
  isEditMode: boolean
  hasChanges: boolean
  onEdit: () => void
  onSave: () => void
...
export function PermissionMatrix({
  ...
  // CRUD operations for each sub-module
  const crudOperations = [
    { id: "view", name: "VIEW", color: "text-blue-600" },
    { id: "add", name: "ADD", color: "text-green-600" },
  ...
  const hasPermission = (employee: Employee, moduleId: string, subModuleId: string, operation: string) => {
    ...
    const subModulePermissions = modulePermissions[subModuleId]
    if (!subModulePermissions) return false

    return subModulePermissions[operation] || false
  }
            ...
            <thead>
              <tr>
                ...
                {modules.map((module) => (
                  <th
                    key={module.id}
                    colSpan={(module.subModules?.length || 0) * crudOperations.length}
            ...
            <tbody>
              {filteredEmployees.map((employee, index) => (
                <tr key={employee.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  ...
                  {/* Permission Checkboxes for each Sub-module and CRUD operation */}
                  {modules.map((module) =>
                    (module.subModules || []).map((subModule) =>
                      crudOperations.map((operation) => {
