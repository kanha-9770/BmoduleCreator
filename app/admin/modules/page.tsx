...
export default function HomePage() {
  ...
  useEffect(() => {
    // Filter and sort modules based on search query, sort order, and permissions
    ...
    // Filter by permissions - only show modules user has explicit access to
    if (!isSystemAdmin && user) {
      const filterByPermissions = (modules: FormModule[]): FormModule[] => {
        return modules
          .filter((module) => hasModuleAccess(module.id))
          .map((module) => ({
            ...module,
            children: module.children ? filterByPermissions(module.children) : [],
          }))
          .filter((module) => hasModuleAccess(module.id) || module.children?.length > 0)
      }
      updatedModules = filterByPermissions(updatedModules)
    }
  ...
  }, [modules, searchQuery, sortOrder, permissions, isSystemAdmin, user])
  ...
  const fetchModules = async () => {
    try {
      setLoading(true)
      // Use ApiClient to ensure proper authentication and permission filtering
      const response = await ApiClient.get("/api/modules", user?.id, user?.email)

      if (response.success) {
        console.log("Fetched modules with permissions:", response.data)
        setModules(response.data)
        setFilteredModules(response.data)
        buildParentOptions(response.data)
  ...
  const buildParentOptions = (moduleList: FormModule[]) => {
    const flattenModules = (modules: FormModule[], level = 0): ParentModuleOption[] => {
      ...
      modules.forEach((module) => {
        ...
        if (isSystemAdmin || hasModuleAccess(module.id)) {
  ...
  const renderModuleAccordion = (modules: FormModule[], level = 0) => {
    return modules
      .map((module) => {
        ...
        const hasAccess = isSystemAdmin || hasModuleAccess(module.id)
      ...
      .filter(Boolean) // Filter out null values (modules without access)
  }
          ...
          <div className="w-72 bg-white border-r border-gray-200 shadow-sm flex flex-col">
            <div className="p-4 space-y-4 flex-shrink-0">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-md p-2 text-xs text-blue-700">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="font-medium">Permission Filtering Active</span>
                </div>
                <p>Only showing modules you have explicit access to</p>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto px-4 pb-4">
              {filteredModules.length ? (
                ...
                <div className="text-center text-gray-500 py-4">No accessible modules found</div>
              )}
            </div>
          </div>
          {/* Main Content Area */}
