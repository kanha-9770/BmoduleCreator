...
export function PermissionProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadUserContext = async () => {
    try {
      ...
      console.log("[PermissionContext] User context loaded:", {
        ...
        isSystemAdmin: userPermissions?.some((p: any) => p.isSystemAdmin) || false,
      })
  ...
  const hasFormAccess = (formId: string): boolean => {
    ...
    const formWithResource = permissions.find((p) => p.resourceType === "form" && p.resourceId === formId)
    if (formWithResource?.resource?.moduleId) {
      const modulePermission = permissions.find(
        (p) => p.resourceType === "module" && p.resourceId === formWithResource.resource!.moduleId,
      )
  ...
  const getAccessibleActions = (moduleId: string, formId?: string) => {
    ...
    if (formId) {
      ...
      if (formPermission) {
        return {
          canView: formPermission.permissions.canView || formPermission.permissions.canManage,
          canAdd: formPermission.permissions.canCreate || formPermission.permissions.canManage,
          canEdit: formPermission.permissions.canEdit || formPermission.permissions.canManage,
          canDelete: formPermission.permissions.canDelete || formPermission.permissions.canManage,
    ...
    if (modulePermission) {
      return {
        canView: modulePermission.permissions.canView || modulePermission.permissions.canManage,
        canAdd: modulePermission.permissions.canCreate || modulePermission.permissions.canManage,
        canEdit: modulePermission.permissions.canEdit || modulePermission.permissions.canManage,
        canDelete: modulePermission.permissions.canDelete || modulePermission.permissions.canManage,
